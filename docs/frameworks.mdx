...
Astro is the same on the plugin (wire it with `inject: false` if you want source pointers on JSX islands) and uses a different channel for the pairing token. `vite.define` does not reach Astro's client pipeline on 7.2+, so the identifier stays literal and the bridge refuses the dial. `init` reads the token in frontmatter and puts it on a `<meta name="reticle-pairing-token">`. Do not use `is:inline` plus `define:vars` for this: that pair can skip the injection. Do not `await import('@reticlehq/react')` from a page `<script>`: that bare dynamic import 404s while Vite's dep cache is cold.

If the app already has `@astrojs/react`, mount a dev-only island. Vite owns that entry the same way it owns any other island, so the SDK is a static import:
...
`init` writes the same `<meta>` tags and a local `src/components/ReticleDev.ts` that a processed `<script>` statically imports, so a minimal Astro app (no React integration) still connects. It also raises `vite.build.target` to `es2022` (Astro's default down-levels the SDK) and keeps `.reticle/` out of the watcher. Static `.astro` markup still gets DOM, network and routing without the React integration.
