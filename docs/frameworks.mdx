...
Astro is the same on the plugin — wire it with `inject: false` if you want source pointers on JSX islands — and a different channel for the pairing token. `vite.define` does not reach Astro's client pipeline on 7.2+, so the identifier stays literal and the bridge refuses the dial. `init` reads the token in frontmatter and hands it over with `define:vars` on an `is:inline` script; the hoisted module then reads `window.__RETICLE_TOKEN__`. Do not put those `<script>` tags inside a conditional: Astro hoists them statically, and a `{isDev && (<script />)}` never renders.
...
`init` also raises `vite.build.target` to `es2022` (Astro's default down-levels the SDK) and keeps `.reticle/` out of the watcher. Static `.astro` markup still gets DOM, network and routing without the React integration.
